...
import { Account, Goal, GoalStatus, Memory, Participant, UUID } from './types';
...
export class BunSqliteDatabase {
    ...
    async getCache(params: { key: string; agentId: string }): Promise<string | undefined> {
        const cached = this.db.prepare(
            "SELECT value FROM cache WHERE key = ? AND agentId = ?"
    ...
    async setCache(params: { key: string; agentId: string; value: string }): Promise<boolean> {
        try {
            this.db.prepare(
                "INSERT OR REPLACE INTO cache (key, agentId, value) VALUES (?, ?, ?)"
    ...
    async deleteCache(params: { key: string; agentId: string }): Promise<boolean> {
        try {
            this.db.prepare(
                "DELETE FROM cache WHERE key = ? AND agentId = ?"
...
async log(params: { body: { [key: string]: unknown }, userId: string, roomId: string, type: string }): Promise<void> {
    const id = uuidv4(); // Use uuidv4 instead of stringToUuid
    this.db.prepare(`
...
async createMemory(memory: Memory, tableName: string): Promise<void> {
    try {
        ...
        console.error('Memory creation failed:', {
            ...
            type: memory.type,  // Log the type to help debug
            error
        });
        throw error;
    }
    ...
    try {
        ...
        return results.map(memory => ({
            ...
            embedding: memory.embedding 
                ? JSON.parse(memory.embedding as unknown as string) 
                : undefined
        }));
    } catch (error) {
...
async getMemories(params: {
    ...
    // Implement other required methods... You can add implementations for other required methods as needed
...
async getCachedEmbeddings(params: {
    ...
    try {
        ...
        console.log('Parameters:', {
            searchTerm: `%${params.query_input}%`,
            matchCount: params.query_match_count
        });

        const results = this.db.prepare(query).all(
            `%${params.query_input}%`,
            params.query_match_count
        ) as { embedding: string, match_length: number }[];

        return results.map(result => ({
            embedding: result.embedding ? JSON.parse(result.embedding) : [],
            levenshtein_score: result.match_length
        })).filter(result => Array.isArray(result.embedding));

    } catch (error) {
        console.error('Error in getCachedEmbeddings:', {
            error,
            params
        });
        return [];
...
async removeAllGoals(roomId: UUID): Promise<void> {
    try {
        this.db.prepare(
            "DELETE FROM goals WHERE roomId = ?"
        ).run(roomId);
    } catch (error) {
...
async updateGoalStatus(params: { goalId: UUID; status: GoalStatus }): Promise<void> {
    try {
        ...
        ).run(params.status, params.goalId);
    } catch (error) {
